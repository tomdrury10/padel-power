...
// Only one sweep runs at a time (job_claim), so a nudge and the cron job
// cannot add the same team twice.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ---- sweep ----
    if ((await rpc("job_claim", { p_name: LOCK, p_seconds: 120 })) !== true) {
    ...
    try {
      ...
      for (const r of regs) {
        ...
        try {
          const teamId = await addTeam(await token(), lid, members.map((m) => String(m.playtomic_player_id)));
          const now = new Date().toISOString();
          for (const m of members) {
            await patchReg(m.id, { playtomic_team_id: teamId, playtomic_added_at: now, playtomic_error: null });
            await log(m.id, "playtomic_added", { team_id: teamId, by: "auto", players: members.map((x) => x.id) });
          }
          done.push({ ids: members.map((m) => m.id), team: teamId });
        } catch (e) {
          const msg = String((e as Error).message).slice(0, 300);
          for (const m of members) {
            await patchReg(m.id, { playtomic_error: msg });
            await log(m.id, "playtomic_failed", { error: msg });
          }
      ...
      await rpc("job_release", { p_name: LOCK }).catch(() => {});
    }
  } catch (e) {
